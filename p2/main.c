...
int Senal(char * sen)
{
/*devuel el numero de senial a partir del nombre*/int i;
...
void cmdBack(char * aux[]){
    ...
    job.pid=pidd;
    job.priority = getpriority(PRIO_PROCESS,job.pid);
    strcpy(job.comando,backcomando);
    strcpy(job.fecha,fecha);
    strcpy(job.user,getUserName());
    insertItemj(job,&jobs);
...
void cmdEjecas(){
    char *aux[100] = {"uid", "-set","-l", trozos[1]};
    cmdUid(aux);
    execvp(trozos[2],&trozos[2]);
}
void cmdFgas(){
...
void cmdListjobs(){
  ...
  for(aux= firstj(jobs);nextj(aux,jobs)!=NULLt;aux=nextj(aux,jobs)) {
      printf("COSITAS\n");
      auxx = getItemj(aux, jobs);
      if (waitpid(auxx.pid, &status, WNOHANG | WUNTRACED | WCONTINUED) == auxx.pid) {
          printf("COSITASIF\n");
          if (WIFCONTINUED(status) == true) {
              printf("COSITASCONTINUE\n");
              printf("%d    %d     %s     %s     %s\n", auxx.pid, auxx.priority, auxx.fecha,
                     auxx.user, "RUNNING");
              ...
              printf("%d    %d     %s     %s     %s      %d\n", auxx.pid, auxx.priority, auxx.fecha,
                     auxx.user,
                     "TERMINATED NORMALLY", state);
          } else if (WIFSIGNALED(status) == true) {
              printf("COSITASSIGNALED\n");
              state = WTERMSIG(status);
              printf("%d    %d    %s      %s     %s     %s\n", auxx.pid, auxx.priority, auxx.fecha,
                     auxx.user,
                     "TERMINATED BY SIGNAL", NombreSenal(state));
          }
...
void cmdJobs(){
    ...
    if(trozos[1]!=NULL) {
        for(aux= firstj(jobs);nextj(aux,jobs)!=NULLt;aux=nextj(aux,jobs)) {
            auxx = getItemj(aux, jobs);
            if (auxx.pid == atoi(trozos[2])) {
                if (strcmp(trozos[1], "-fg") == 0) {
                    char *aux[100] = {"fg", trozos[2]};
                    cmdFg(aux);
                    deleteAtPositionj(aux, &jobs);
                } else
                    printf("%d    %d    %s      %s     %d     %s\n", auxx.pid, auxx.priority, auxx.fecha,
                                  auxx.user, auxx.state, auxx.termination);
            }
        }
    }
    else
...
int procesarEntrada(){
    if(trozos[0]==NULL){
        ...
        if(strncmp(trozos[0],"autores",8)==0){
        ...
        else if(strncmp(trozos[0],"fgpri",5)==0){
            cmdFgpri();
        }
        else if(strcmp(trozos[0],"back")==0){
            cmdBack(trozos);
        }
        else if(strncmp(trozos[0],"backpri",7)==0){
            cmdBackpri();
        }
        else if(strncmp(trozos[0],"ejecas",6)==0){
            cmdEjecas();
        }
        else if(strncmp(trozos[0],"backas",6)==0){
            cmdBackas();
        }
        else if(strncmp(trozos[0],"fgas",4)==0){
            cmdFgas();
        }
        else if(strncmp(trozos[0],"listjobs",8)==0){
            cmdListjobs();
        }
        else if(strncmp(trozos[0],"borrarjobs",10)==0){
            cmdBorrarjobs();
        }
            ...
            int index=0;
            char *prog [100];
            while(trozos[index]!=NULL) {
                if (strcmp(trozos[index], "&") == 0) {
                    prog[0] = "back";
                    cmdBack(prog);
                } else {
                    prog[index + 1] = trozos[index];
                }
                index++;
            }
            prog[0]= "fg";
